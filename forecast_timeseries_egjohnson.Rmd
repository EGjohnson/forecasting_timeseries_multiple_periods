...
forecast.date<-seq(from=ymd_hms(start.new.time.vec),length.out=dim(the.forecast %>% data.frame)[1] %>% length,by="1 min")
...
# function to randomize events while preserving day/week structure
reshuffle.spikes<-function(shift.sec){tcdat %>% filter(log.response>2.5) %>% mutate(wkd=weekdays(begin_time)) %>% group_by(wkd) %>% 
  mutate(average_response_time=sample(average_response_time,size=length(average_response_time),replace=T)) %>% 
    mutate(begin_time=begin_time+shift.sec) %>% ungroup}
...
```{r}
ggplot(the.forecast.adj,aes(forecast.date,Point.Forecast.resptime))+geom_line()+
  scale_x_datetime(breaks = date_breaks("1 day"), labels=date_format("%a-%d",tz=Sys.timezone()))+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
```{r}
ggplot(the.forecast.adj,aes(forecast.date,Point.Forecast))+geom_line(size =0.1) +
  scale_x_datetime(breaks = date_breaks("1 day"), labels=date_format("%a-%d",tz=Sys.timezone()))+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r}
the.forecast.adj %>% mutate(begin_time=as.numeric(forecast.date)) %>% select(begin_time,Point.Forecast.resptime,Point.Forecast) %>% 
  rename(average_response_time=Point.Forecast.resptime,throughput=Point.Forecast) %>% 
  head
```