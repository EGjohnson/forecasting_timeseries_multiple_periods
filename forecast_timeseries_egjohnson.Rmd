...
the.forecast.adj<-rbind(the.forecast %>% filter(dst=="yes"),the.forecast.dst.shift)

```

```{r}
p1 %>% hello
p1 <- ggplot(the.forecast.adj %>% filter(weekdays.POSIXt()), aes(forecast.date,Point.Forecast))+
