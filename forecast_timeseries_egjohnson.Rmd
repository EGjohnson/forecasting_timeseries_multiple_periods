...
forecast.date<-seq(from=ymd_hms(start.new.time.vec),length.out=the.forecast$mean %>% length,by="1 min")
...
```{r}
ggplot(the.forecast.adj,aes(forecast.date,throughput.impact))+geom_line(size =0.1) +
  scale_x_datetime(breaks = date_breaks("1 day"), labels=date_format("%a-%d",tz=Sys.timezone()))+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
