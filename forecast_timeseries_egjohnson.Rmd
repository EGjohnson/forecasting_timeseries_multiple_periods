...
the.forecast.adj %>% mutate(resp_p2_component=10^resp_p2_component_log) %>% select(-resp_p2_component_log) %>%
mutate(resp_p2_component=resp_p2_component+4*sample(10^(response.throughput.mod$residuals),size=length(the.forecast.adj$resp_p2_component),replace=TRUE))->the.forecast.adj
```


### account for process 1 (randomize response times while keeping the day specific structure)
...
reshuffle.spikes<-function(shift.sec){tcdat %>% filter(log.response>2.5) %>% mutate(wkd=weekdays(begin_time)) %>% group_by(wkd) %>% 
...
#events randomized
response.spikes.randomized<-rbind(reshuffle.spikes(60*60*24*7*2),reshuffle.spikes(60*60*24*7*4)) %>% select(begin_time,average_response_time) %>% rename(average_response_time_spike=average_response_time,forecast.date=begin_time)

# 2. merge strucutured random spike "events" into forecasted data frame 
merge(response.spikes.randomized,the.forecast.adj,by="forecast.date",all=T) %>% arrange(forecast.date) -> the.forecast.adj

# 3. combine both process 1 and 2 to get simulated response timecourse
the.forecast.adj %<>% 
...
```{r}
ggplot(the.forecast.adj,aes(forecast.date,resp_p2_component))+geom_line()
